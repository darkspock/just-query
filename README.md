...
FastPHP QueryBuilder is a fork of the excellent [yiisoft/db](https://github.com/yiisoft/db) and [yiisoft/db-mysql](https://github.com/yiisoft/db-mysql) libraries.
...
```php
use FastPHP\QueryBuilder\Driver\Pgsql\Connection;
...
Full list of condition operators: `=`, `!=`, `<>`, `>`, `>=`, `<`, `<=`, `BETWEEN`, `NOT BETWEEN`, `IN`, `NOT IN`, `LIKE`, `NOT LIKE`, `OR LIKE`, `OR NOT LIKE`, `EXISTS`, `NOT EXISTS`, `ARRAY OVERLAPS`, `JSON OVERLAPS`, `AND`, `OR`, `NOT`.
...
$command->setRetryHandler(function (\FastPHP\QueryBuilder\Exception\Exception $e, int $attempt): bool {
    // $attempt starts at 1
    if ($attempt > 3) {
...
use FastPHP\QueryBuilder\Expression\Statement\WhenThen;
...
PDO returns everything as strings. FastPHP casts automatically based on schema:
