...
abstract class AbstractCommand implements CommandInterface
{
    ...
    public function increment(
        ...
        return $this->incrementEach($table, [$column => $amount], $condition, $extra);
    }
    ...
    public function incrementEach(
        ...
        foreach ($columns as $col => $amount) {
            $quotedCol = $this->db->getQuoter()->quoteColumnName($col);
            $sign = $amount >= 0 ? '+' : '-';
            $absAmount = abs($amount);
            $updates[$col] = new Expression("$quotedCol $sign $absAmount");
        }

        return $this->update($table, $updates, $condition);
    }
