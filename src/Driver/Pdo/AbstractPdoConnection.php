...
use Throwable;
use JustQuery\Cache\NullCache;
use JustQuery\Cache\SchemaCache;
use JustQuery\Connection\AbstractConnection;
...
use JustQuery\Profiler\Context\ConnectionContext;
use JustQuery\Profiler\ProfilerAwareInterface;
use JustQuery\Profiler\ProfilerAwareTrait;
...
abstract class AbstractPdoConnection extends AbstractConnection implements PdoConnectionInterface, LoggerAwareInterface, ProfilerAwareInterface
{
    ...
    public function open(): void
    {
        ...
        if ($this->sharedPdo instanceof PDO) {
            $this->pdo = $this->sharedPdo;
            return;
        }
    ...
    public function setSharedPdo(PDO $pdo): void
    {
        if ($this->pdo !== null && $this->pdo !== $pdo && !$this->isUsingSharedPdo()) {
            $this->close();
        }
    ...
    protected function isUsingSharedPdo(): bool
    {
        return $this->sharedPdo !== null && $this->pdo === $this->sharedPdo;
    }

    protected static function createDefaultSchemaCache(): SchemaCache
    {
        return new SchemaCache(new NullCache());
    }
