...
class Query implements QueryInterface
{
    ...
    public function whereIntegerInRaw(string $column, array $values): static
    {
        ...
        $quotedColumn = $this->db->getQuoter()->quoteColumnName($column);
        $safeValues = implode(', ', array_map('intval', $values));

        return $this->andWhere(new \JustQuery\Expression\Expression(
            "$quotedColumn IN ($safeValues)"
        ));
    }
    ...
    public function whereIntegerNotInRaw(string $column, array $values): static
    {
        ...
        return $this->andWhere(new \JustQuery\Expression\Expression(
            "$quotedColumn NOT IN ($safeValues)"
        ));
    }
    ...
    public function chunkById(int $size, Closure $callback, string $column = 'id'): bool
    {
        ...
        do {
            ...
            if (is_array($lastRow) && isset($lastRow[$column])) {
