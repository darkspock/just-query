...
interface QueryPartsInterface
{
     ...
     * $query = (new \FastPHP\QueryBuilder\Query\Query)->from(['u' => 'user', 'profile']);
     *
     * // SELECT * FROM (SELECT * FROM `user` WHERE `active` = 1) `activeusers`;
     * $subQuery = (new \FastPHP\QueryBuilder\Query\Query)->from('user')->where(['active' => true])
     * $query = (new \FastPHP\QueryBuilder\Query\Query)->from(['activeusers' => $subQuery]);
     *
