...
interface QueryPartsInterface
{
     ...
     * $query = (new \JustQuery\Query\Query)->from(['u' => 'user', 'profile']);
     *
     * // SELECT * FROM (SELECT * FROM `user` WHERE `active` = 1) `activeusers`;
     * $subQuery = (new \JustQuery\Query\Query)->from('user')->where(['active' => true])
     * $query = (new \JustQuery\Query\Query)->from(['activeusers' => $subQuery]);
     *
    ...
    /**
     * Adds a `WHERE column IN (...)` condition with raw integer values (no PDO bindings).
     *
     * This is significantly faster than `['in', $column, $values]` for large lists of integer IDs
     * because it skips PDO parameter binding entirely. Safe because integers cannot cause SQL injection.
     *
     * ```php
     * $query->whereIntegerInRaw('id', [1, 2, 3, 4, 5]);
     * // WHERE `id` IN (1, 2, 3, 4, 5)  — no :qp0, :qp1, ... bindings
     * ```
     *
     * @param string $column The column name.
     * @param array<int|string> $values The integer values.
     */
    public function whereIntegerInRaw(string $column, array $values): static;

    /**
     * Adds a `WHERE column NOT IN (...)` condition with raw integer values (no PDO bindings).
     *
     * @param string $column The column name.
     * @param array<int|string> $values The integer values.
     */
    public function whereIntegerNotInRaw(string $column, array $values): static;
